...
import NoNotificationsFound from "../components/NoNoificationsFound"
...
const Notifications = () => {
  ...
  const { mutate: acceptRequestMutation } = useMutation({
    ...
    onMutate: async (requestId) => {
      ...
      // Optimistically update - remove the request immediately
      queryClient.setQueryData(["friendRequests"], (old) => ({
        ...old,
        incomingRequests: old?.incomingRequests?.filter(req => req._id !== requestId) || []
      }));
